...
var fastQueryPageResults = new Array();
...
function goToHomePage() {
    window.location.href = "http://azuresulphur1.redmond.corp.microsoft.com/CaptureUIsReport/";
}


$(document).ready(function () {
    QueryClear();
    $(SIPromptMessage).show().html("click head picture show fast query, click here show condition query!");
    $("#Prompt-Message").removeClass("disabled");
...
function FastTableQuery() {
    flag = "fastQuery";
    pageIndex = 1;
    console.log("enter fast query");
    SelectedSortBy = "Build No.";
    QueryClear();
    $(SIPromptMessage).show().html("Loading...");
    $("#Prompt-Message").addClass("disabled");
    document.getElementById('ckbReviewBugs').checked = false;
    $.ajax({
        type: "POST",
        url: "IndexService.asmx/GetFastQueryNumber",
        data: "{alias:\"" + alias + "\"}",
        contentType: "application/json; charset=utf-8",
        error: function (data) {
        ...
        success: function (data) {
            ...
            if (data.d == 0) {
                $("#divLoading").hide();
                $(SIPromptMessage).show().html('No records found according to the conditions. Click here execute condition query.');
                $("#BackToCondition").removeClass('hide');
                QueryClear();
            } else {
                FastTableQueryResults(pageIndex);
            }
        }
    });
}

function FastTableQueryResults(pageIndex) {
    console.log("enter FastTableQueryResults");
    $("#BackToCondition").removeClass('hide');
    $('.Collapse').addClass('hide');
    $('#next').addClass('hide');
    GlobalResultIDList = "";
    ...
    $.ajax({
        type: "POST",
        url: "IndexService.asmx/GetFastQueryResultList",
        data: "{alias:\"" + alias + "\",pageIndex:\"" + pageIndex + "\",currentPageResults:\"" + fastQueryPageResults[pageIndex] + "\"}",
        contentType: "application/json; charset=utf-8",
        error: function (data) {
            alert("Fail to query records!");
        },
        success: function (data) {
            GenerateRowElements(data, pageIndex);
        }
    });
}
...
function ConditionQuery() {
    flag = "conditionQuery";
    pageIndex = 1;
    console.log("enter condition query ");
    SelectedSortBy = "Build No.";
...
function Search() {
    ...
    if (searchValue == null || searchValue == "") {
        alert("search box must have a value");
        return;
    } else {
        QueryClear();
        $("#BackToCondition").removeClass('hide');
...
// have some problem to be repaired
...
function GenerateRowElements(data, pageIndex) {
    if (data.d.length == 0) {
        QueryClear();
        $(SIPromptMessage).show().html('No records found according to the conditions. Click here execute condition query.');
        $("#Prompt-Message").removeClass("disabled");
        ...
        if (lastPage != pages && data.d.length == 20) {
            if (flag == "search") {
                if (pageIndex == 1 && searchPages > 1) {
                    $('#next').removeClass('hide');
                }
                else if (pageIndex > 1 && pageIndex < searchPages) {
                    $('#Previous').removeClass('hide');
                    $('#next').removeClass('hide');
                }
                else if (pageIndex == searchPages) {
                    $('#Previous').removeClass('hide');
                }
            }

            else if (flag == "conditionQuery") {
                if (pageIndex == 1 && conditionPages > 1) {
                    $('#next').removeClass('hide');
                }
                else if (pageIndex > 1 && pageIndex < conditionPages) {
                    $('#Previous').removeClass('hide');
                    $('#next').removeClass('hide');
                }
                else if (pageIndex == conditionPages) {
                    $('#Previous').removeClass('hide');
                }
            }
            else if (flag == "fastQuery") {
                if (pageIndex == 1 && fastQueryPages > 1) {
                    $('#next').removeClass('hide');
                }
                else if (pageIndex > 1 && pageIndex < fastQueryPages) {
                    $('#Previous').removeClass('hide');
                    $('#next').removeClass('hide');
                }
                else if (pageIndex == fastQueryPages) {
                    $('#Previous').removeClass('hide');
                }
            }
        } else {
        ...
        if (flag == "search" && searchPageResults[pageIndex] == null) {
            searchPageResults[pageIndex] = GlobalResultIDList.substring(0, GlobalResultIDList.length - 1);
        } else if (flag == "conditionQuery" && conditionPageResults[pageIndex] == null) {
            conditionPageResults[pageIndex] = GlobalResultIDList.substring(0, GlobalResultIDList.length - 1);
        } else if (flag == "fastQuery" && fastQueryPageResults[pageIndex] == null) {
            fastQueryPageResults[pageIndex] = GlobalResultIDList.substring(0, GlobalResultIDList.length - 1);
        }
...
function EnablePageNavigation(totalCount, index) {
    ...
    $('#PageIndexs .PageTag').click(function () {
        ...
        if (flag == "search") {
            ...
            FastTableQueryResults(currentPage);
        }
    });
    if (index == 0) {
...
function BackTocondition() {
    ...
    var isExecuteConditionQuery = false;
    if (conditionPages == undefined) {
        isExecuteConditionQuery = true;
        ConditionQuery();
    }
