...
namespace SulpHurReport
{
    ...
    public class IndexService : System.Web.Services.WebService
    {
        ...
        public int UpdateReviewedByResultID(string resultID, string reviewflag, string reviewlog)
        {
            ...
            SQLUtility.ExecuteScalar("Update ResultsInfoByAlias set ReviewFlag='"+ int.Parse(reviewflag) + "'"+"where ResultID in (" + resultID.Trim(',') + ")",true);
        ...
        [WebMethod]
        public int GetFastQueryNumber(string alias)
        {
            int count = 0;
            string sql = string.Format("select count(resultID) from ResultsInfoByAlias where alias = '{0}'", alias);
            count = SQLUtility.ExecuteScalar(sql, false);
            return count;
        }

        [WebMethod]
        public List<QueryResult> GetFastQueryResultList(string alias,string pageIndex,string currentPageResults)
        {
            //string sql = string.Format("select * from ResultsInfoByAlias where alias = '{0}' order by buildNO", alias);
            //List<QueryResult> resultList = new List<QueryResult>();
            //resultList = SQLUtility.ExecuteScalar(ConfigurationManager.ConnectionStrings["ADOConn"].ToString(), sql, false);
            //return resultList;
            int pageSize = 20;
            int offset = (int.Parse(pageIndex) - 1) * pageSize;
            string sql = string.Empty;
            if (currentPageResults != "undefined")
            {
                sql = string.Format("select * from ResultsInfoByAlias where resultID in ({0})", currentPageResults);
            }
            else
            {
                sql = string.Format("SELECT * FROM ResultsInfoByAlias WHERE alias = '{0}' ORDER BY buildNO DESC " +
                                         "OFFSET {1} ROWS FETCH NEXT {2} ROWS ONLY", alias, offset, pageSize);
            }
            List<QueryResult> resultList = new List<QueryResult>();
            resultList = SQLUtility.ExecuteScalar(ConfigurationManager.ConnectionStrings["ADOConn"].ToString(), sql, false);
            return resultList;
