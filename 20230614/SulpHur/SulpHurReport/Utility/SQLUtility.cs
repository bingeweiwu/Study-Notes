...
namespace SulpHurReport.Utility
{
 ...
 AND [TOTAL].[UIName]=[GroupTable].[UIName]";
        ...
        public static string sqlResultsOSRelatedLatest = @"SELECT ResultID, buildNo, Language, RuleName, ResultType, UIName, UserName, OSType, DateUploaded, CreateDate, ReviewFlag FROM (
 ...
 AND [TOTAL].[Language]=[GroupTable].[Language]) AS newT WHERE rowNum BETWEEN {2} AND {3} ORDER BY rowNum";
        ...
        public static string sqlResultsOSNotRelatedLatest = @"SELECT ResultID, buildNo, Language, RuleName, ResultType, UIName, UserName, OSType, DateUploaded, CreateDate, ReviewFlag FROM (
 SELECT ROW_NUMBER() OVER (ORDER BY [TOTAL].[BuildNo] DESC, [TOTAL].[ResultID] DESC) AS rowNum, [TOTAL].[ResultID], [TOTAL].[BuildNo], 
 [TOTAL].[Language], [TOTAL].[RuleName], [TOTAL].[ResultType], [TOTAL].[UIName], [TOTAL].[UserName], [TOTAL].[OSType], [TOTAL].[DateUploaded], 
 [TOTAL].[CreateDate], [TOTAL].[ReviewFlag] FROM (
 SELECT DISTINCT [BuildInfo].[BuildNo], [Results].[ResultID], [Rules].[RuleName], [UIContents].[UIName], [Clients].[UserName], 
 ...
 drop table #TempGroupTable";
        ...
        public static string sqlResultsOSRelatedWithAssemblyLatest = @"SELECT  ResultID, buildNo, Language, RuleName, ResultType, UIName, UserName, OSType, DateUploaded, CreateDate, ReviewFlag  FROM (
...
SELECT * FROM (
 ...
 drop table #TempGroupTable";
        ...
        public static string sqlResultsOSNotRelatedWithAssemblyLatest = @"SELECT  ResultID, buildNo, Language, RuleName, ResultType, UIName, UserName, OSType, DateUploaded, CreateDate, ReviewFlag  FROM (
 SELECT ROW_NUMBER() OVER (ORDER BY [BuildNo] DESC, [ResultID] DESC) AS rowNum, * FROM (
 SELECT DISTINCT [TOTAL].[BuildNo], [TOTAL].[ResultID], [TOTAL].[UIName], [TOTAL].[ResultType], [TOTAL].[ReviewFlag], [TOTAL].[Language],
 [TOTAL].[RuleName],[TOTAL].[UserName],[TOTAL].[OSType],[TOTAL].[DateUploaded],[TOTAL].[CreateDate]
 FROM (
 ...
 WHERE 1=1 {0}";
        ...
        public static string sqlResultsCommon = @"SELECT  ResultID, buildNo, Language, RuleName, ResultType, UIName, UserName, OSType, DateUploaded, CreateDate, ReviewFlag  FROM (
 ...
 WHERE 1=1 {0}) AS newT WHERE rowNum BETWEEN {1} AND {2} ORDER BY rowNum";
        ...
        public static string sqlResultsCommonWithAssembly = @"SELECT  ResultID, buildNo, Language, RuleName, ResultType, UIName, UserName, OSType, DateUploaded, CreateDate, ReviewFlag  FROM (
 SELECT ROW_NUMBER() OVER (ORDER BY [BuildNo] DESC, [ResultID] DESC) AS rowNum, * FROM (
 ...
 WHERE [Results].[ResultID] IN ({0}) ORDER BY [BuildInfo].[BuildNo] DESC, [Results].[ResultID] DESC";
        ...
        public static string GenerateReviewedTypeFilterClause(string reviewedType)
        {
            ...
            else if (reviewedType == "'Reviewed'") reviewedType = "1";
            else if (reviewedType == "All") reviewedType = "-1";
            if (reviewedType != "-1")
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        public static string GenerateCountSQLByResultID(string resultId)
        {
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        public static string GenerateListSQLByResultID(string resultId)
        {
            string sql = string.Format(@"select  ResultID,buildNo,Language,RuleName,ResultType,UIName,UserName,OSType,DateUploaded,CreateDate,ReviewFlag from [Results]
...
WHERE[Results].[ResultID] IN({0})",
        ...
        public static int GetCountOfTotalRecords(string sql)
        {
        ...
        public static int GetCountOfTotalRecords(string buildNO, string buildLan, string osType, string osLanguage, string ruleid,
            string resultTypes, string assemblyName, string typeID,
            string ReviewedStatus, string searchDateTime, bool getLatest, int latestDays)
        {
            string whereClauseCommon = string.Format("{0}{1}{2}{3}{4}{5}",
            ...
            string whereClauseResultTypeAndReviewType = string.Format("{0}{1}",
                ...
                SQLUtility.GenerateReviewedTypeFilterClause(ReviewedStatus));
            ...
            else
            {
                ...
                //    username, pagetitle, reviewComments, ReviewedStatus, searchDateTime);
                if (assemblyName == "All" && typeID == "-1")
                {
        ...
        public static List<QueryResult> GetDetailResultsOfSpecifiedPage(string buildNO, string buildLan, string osType, string osLanguage,
            string ruleid, string resultTypes, string assemblyName, string typeID, string ReviewedType, string sortBy, string searchDateTime, string currentPageResults,
            string diffentReviewStatusResultsCount, bool getLatest, int latestDays, string pageIndex)
        {
            string whereClauseCommon = string.Format("{0}{1}{2}{3}{4}{5}",
                SQLUtility.GenerateBuildNOFilterClause(buildNO),
                SQLUtility.GenerateBuildLanFilterClause(buildLan),
            ...
            if (ReviewedType == "'UnReviewed'" || ReviewedType == "'Reviewed'")
            {
            ...
            string sql = string.Empty;
            if (currentPageResults != "undefined")
            {
            ...
            resultList = ExecuteScalar(connStr, sql, false);
            return resultList;
        }
        ...
        {
            SulpHurEntitiesForReport entities = new SulpHurEntitiesForReport();
...
order by RuleID,Language,maxBuildNo desc";
        ...
        public static int ExecuteScalar(string sql, bool isSearch)
        {
            SqlConnection conn = new SqlConnection(connStr);
            ...
            int number = objResult == null ? 0 : Convert.ToInt32(objResult);
            return number;
        }
        ...
        public static List<QueryResult> ExecuteScalar(string connStr, string sql, bool isSearch)
        {
            SqlConnection conn = new SqlConnection(connStr);
            SqlCommand command = new SqlCommand(sql, conn);
            command.CommandType = CommandType.Text;
