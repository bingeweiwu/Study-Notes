...
namespace MS.Internal.SulpHur.CMRules
{
    public class TruncationRule : UIComplianceRuleBase
    {
        ...
        static List<TextTruncationRuleFilter> textTruncationRuleFilterList;
        ...
        static TruncationRule()
        {
            SulpHurEntitiesForCMRules sulpHurEntitiesForCMRules = new SulpHurEntitiesForCMRules();
            textTruncationRuleFilterList = sulpHurEntitiesForCMRules.TextTruncationRuleFilters.ToList();
        }
        ...
        public override List<UIComplianceResultBase> UIVerify(ICollection<ElementInformation> Controls)
        {
            ...
            try
            {
                ...
                foreach (ElementInformation winInfo in Controls)
                {
                    ...
                    TextTruncationRuleFilter filter = null;
                    if (textTruncationRuleFilterList != null && textTruncationRuleFilterList.Count > 0)
                    {
                        filter = textTruncationRuleFilterList.Where(item =>
                            item.RootAutomationID == winInfo.Ancestors[winInfo.Ancestors.Count - 1].AutomationId
                            && item.ControlAutomationID == winInfo.AutomationId
                            && item.ControlType == winInfo.ControlType.ToString())
                            .FirstOrDefault();
                    }
                    if (filter != null)
                        continue;
