...
namespace MS.Internal.SulpHur.CMRules
{
    public class OverlapRule : UIComplianceRuleBase
    {
        ...
        static List<OverlapRuleFilter> overlapRuleFilterList;
        static List<OverlapRuleChildAndParentFilter> overlapRuleChildAndParentFilterList;
        static ControlType[] falseAlarm_ComparedControlType_Horizontal = {
            ControlType.Button,
        ...
        static ControlType[] falseAlarm_ComparedControlType_Vertical = {
        ...
        static ControlType[] falseAlarm_CheckBoxComparedControlType_Vertical = {
            ControlType.Document
        };
        static ControlType[] falseAlarm_TwoSameControlsControlType_Vertical = {
            ControlType.CheckBox,
            ControlType.Hyperlink,
            ControlType.RadioButton
        };
        ...
        static OverlapRule()
        {
            ...
            overlapRuleFilterList = sulpHurEntitiesForCMRules.OverlapRuleFilters.ToList();
            overlapRuleChildAndParentFilterList = sulpHurEntitiesForCMRules.OverlapRuleChildAndParentFilters.ToList();
        }
        ...
        private List<UIComplianceResultBase> CheckControlOverlap(ElementInformation Parent, List<ElementInformation> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    OverlapRuleChildAndParentFilter childAndParentfilter = null;
                    if (overlapRuleChildAndParentFilterList != null && overlapRuleChildAndParentFilterList.Count > 0)
                    {
                        childAndParentfilter = overlapRuleChildAndParentFilterList.Where(item =>
                            item.RootAutomationID == control.Ancestors[control.Ancestors.Count - 1].AutomationId
                            && item.ChildControlAutomationID == control.AutomationId
                            && item.ChildControlType == control.ControlType.ToString()
                            && item.ParentControlAutomationID == ParentControl.AutomationId
                            && item.ParentControlType == ParentControl.ControlType.ToString())
                            .FirstOrDefault();
                    }

                    if (childAndParentfilter == null && !ParentControl.BoundingRectangle.Contains(control.BoundingRectangle) && !filteredTypes.Contains(ParentControl.ControlType))
                    {
                        ...
                        sb.AppendLine(string.Format("[Name:{0}, ControlType:{1}, Location:{2}] and it's parent [Name:{3}, ControlType:{4}, Location:{5}] are overlap",
                ...
                for (int j = i + 1; j < list.Count; j++)
                {
                    ...
                    OverlapRuleFilter filter = null;
                    if (overlapRuleFilterList != null && overlapRuleFilterList.Count > 0)
                    {
                        filter = overlapRuleFilterList.Where(item =>
                            item.RootAutomationID == control.Ancestors[control.Ancestors.Count - 1].AutomationId
                            && item.FirstControlAutomationID == control.AutomationId
                            && item.FirstControlType == control.ControlType.ToString()
                            && item.SecondControlAutomationID == comparedControl.AutomationId
                            && item.SecondControlType == comparedControl.ControlType.ToString())
                            .FirstOrDefault();
                    }
                    if (filter != null)
                        continue;
                    ...
                    if (Pix0IntersectsWith(comparedControl.BoundingRectangle, control.BoundingRectangle))
                    {
                        ...
                        bool ignoreFalseAlarm = IgnoreFalseAlarm_TextAndTheOtherControl_Horizontal(control, comparedControl);
                        if (ignoreFalseAlarm)
                            continue;
                        ignoreFalseAlarm = IgnoreFalseAlarm_TextAndTheOtherControl_Vertical(control, comparedControl);
                        if (ignoreFalseAlarm)
                            continue;
                        ignoreFalseAlarm = IgnoreFalseAlarm_CheckBoxAndText_Horizontal(control, comparedControl);
                        if (ignoreFalseAlarm)
                            continue;
                        ignoreFalseAlarm = IgnoreFalseAlarm_CheckBoxAndTheOtherControl_Vertical(control, comparedControl);
                        if (ignoreFalseAlarm)
                            continue;
                        ignoreFalseAlarm = IgnoreFalseAlarm_TwoSameControls_Vertical(control, comparedControl);
                        if (ignoreFalseAlarm)
                            continue;
        ...
        private bool IgnoreFalseAlarm_TextAndTheOtherControl_Horizontal(ElementInformation control, ElementInformation comparedControl)
        {
            //skip the controls
            //Text and the other control
            //Text is the previous control of the other control
            //Control Text's text size is shorter 10 pixes (Maybe 10 pixes is incorrect, current use this value)
            if (control.ControlType == ControlType.Text && falseAlarm_ComparedControlType_Horizontal.Contains(comparedControl.ControlType)
                && control.TabIndex + 1 == comparedControl.TabIndex
                && control.TextSize1 != null && control.X + control.TextSize1.Width + 10 <= comparedControl.X)
                return true;
            else if (comparedControl.ControlType == ControlType.Text && falseAlarm_ComparedControlType_Horizontal.Contains(control.ControlType)
                && comparedControl.TabIndex + 1 == control.TabIndex
                && comparedControl.TextSize1 != null && comparedControl.X + comparedControl.TextSize1.Width + 10 <= control.X)
                return true;
            else
                return false;
        }
        private bool IgnoreFalseAlarm_TextAndTheOtherControl_Vertical(ElementInformation control, ElementInformation comparedControl)
        {
            //skip the controls
            //Text and the other control
            //Control Text's text size is shorter 10 pixes (Maybe 10 pixes is incorrect, current use this value)
            if (control.ControlType == ControlType.Text && falseAlarm_ComparedControlType_Vertical.Contains(comparedControl.ControlType)
                && control.TextSize1 != null && control.Y + control.TextSize1.Height + 10 <= comparedControl.Y)
                return true;
            else if (comparedControl.ControlType == ControlType.Text && falseAlarm_ComparedControlType_Vertical.Contains(control.ControlType)
                && comparedControl.TextSize1 != null && comparedControl.Y + comparedControl.TextSize1.Height + 10 <= control.Y)
                return true;
            else
                return false;
        }
        private bool IgnoreFalseAlarm_CheckBoxAndText_Horizontal(ElementInformation control, ElementInformation comparedControl)
        {
            //skip the controls
            //CheckBox and Text
            //Control CheckBox's text size is shorter 10 pixes (Maybe 10 pixes is incorrect, current use this value)
            if (control.ControlType == ControlType.CheckBox && comparedControl.ControlType == ControlType.Text
                && control.TextUnit != null && control.X + control.TextUnit.Width < comparedControl.X)
                return true;
            else if (comparedControl.ControlType == ControlType.CheckBox && control.ControlType == ControlType.Text
                && comparedControl.TextUnit != null && comparedControl.X + comparedControl.TextUnit.Width < control.X)
                return true;
            else
                return false;
        }
        private bool IgnoreFalseAlarm_CheckBoxAndTheOtherControl_Vertical(ElementInformation control, ElementInformation comparedControl)
        {
            //skip the controls
            //CheckBox and the other control
            //Control CheckBox's text size is shorter 10 pixes (Maybe 10 pixes is incorrect, current use this value)
            if (control.ControlType == ControlType.CheckBox && falseAlarm_CheckBoxComparedControlType_Vertical.Contains( comparedControl.ControlType)
                && control.TextUnit != null && control.Y + control.TextUnit.Height + 10 <= comparedControl.Y)
                return true;
            else if (comparedControl.ControlType == ControlType.CheckBox && falseAlarm_CheckBoxComparedControlType_Vertical.Contains(control.ControlType)
                && comparedControl.TextUnit != null && comparedControl.Y + comparedControl.TextUnit.Height + 10 <= control.Y)
                return true;
            else
                return false;
        }
        private bool IgnoreFalseAlarm_TwoSameControls_Vertical(ElementInformation control, ElementInformation comparedControl)
        {
            //skip the controls
            //two same controls
            //Control Text's text size is shorter 10 pixes (Maybe 10 pixes is incorrect, current use this value)
            if ((control.ControlType == comparedControl.ControlType && falseAlarm_TwoSameControlsControlType_Vertical.Contains(control.ControlType))
                && (control.TextSize1 != null && control.Y + control.TextSize1.Height + 10 <= comparedControl.Y
                || comparedControl.TextSize1 != null && comparedControl.Y + comparedControl.TextSize1.Height + 10 <= control.Y))
                return true;
            else
