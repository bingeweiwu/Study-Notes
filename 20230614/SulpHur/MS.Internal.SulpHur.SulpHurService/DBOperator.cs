...
namespace MS.Internal.SulpHur.SulpHurService
{
    public class DBOperator : QueryOperations, IDisposable
    {
        ...
        public static string ChangeBuildNo(string buildNo)
        {
            ...
            if (CMUtility.IsStandardFormat(buildNo))
            {
            ...
            out UploadResults re)
        {
            ...
            try
            {
                ...
                if (!Validation.NotNullOrEmpty(productbuild) ||
                    !CMUtility.IsStandardFormat(productbuild) ||
                ...
                catch (Exception ex)
                {
                    ...
                    if (contentTable.GUID == null || contentTable.GUID.Value == null || string.IsNullOrEmpty(contentTable.GUID.Value.ToString()) || contentTable.BuildID < 0 || contentTable.ClientID < 0)
                        return;
                    //backup the issue data

                    //<100
                    string tempDataFolder = @"F:\SulpHur\SulpHurService\tempData";
                    string[] dirList = Directory.GetDirectories(tempDataFolder);
                    if (dirList.Length > 100) return;

                    string guidFolder = Path.Combine(tempDataFolder, contentTable.GUID.Value.ToString());
                    bool exist = Directory.Exists(guidFolder);
                    if (exist) return;
                    Directory.CreateDirectory(guidFolder);
                    //additional info
                    using (StreamWriter sw = new StreamWriter(Path.Combine(guidFolder, "additionalInfo.txt")))
                    {
                        sw.AutoFlush = true;
                        sw.WriteLine("contentTable.BuildID: " + contentTable.BuildID);
                        sw.WriteLine("contentTable.DateUploaded: " + contentTable.DateUploaded);
                        sw.WriteLine("contentTable.ClientID: " + contentTable.ClientID);
                        sw.WriteLine("contentTable.IsWebUI: " + contentTable.IsWebUI);
                        sw.WriteLine("contentTable.UIName: " + contentTable.UIName);
                        sw.WriteLine("contentTable.LaunchedFrom: " + contentTable.LaunchedFrom);
                        sw.WriteLine("contentTable.WindowHierarchy: " + contentTable.WindowHierarchy);
                    }
                    //xml
                    try
                    {
                        elementTree.Save(Path.Combine(guidFolder, "uicontent.xml"));
                    }
                    catch (Exception e)
                    {
                        Log.WriteServerLog("Generate uicontent.xml failed.", TraceLevel.Error);
                        Log.WriteServerLog("message:" + e.Message, TraceLevel.Error);
                    }
                    //screenshot
                    try
                    {
                        using (MemoryStream ms = new MemoryStream(image.ToArray()))
                        {
                            ...
                            bitmap.Save(Path.Combine(guidFolder, "screenshot.bmp"));
                        }
                    }
                    catch (Exception e)
                    {
                        Log.WriteServerLog("Generate screenshot.bmp failed.", TraceLevel.Error);
                        Log.WriteServerLog("message:" + e.Message, TraceLevel.Error);
                    }
                    return;
                }
        ...
        public List<UIANDRule> QueryAllDirtyUI()
        {
            ...
            {
                Log.WriteServerLog("u.Uiid = " + u.Uiid.ToString());
                verifyList.Add(u);
            }
        ...
        public static byte[] GetResultScreenshot(int resultID)
        {
            ...
            catch (System.Data.MappingException)
            {
