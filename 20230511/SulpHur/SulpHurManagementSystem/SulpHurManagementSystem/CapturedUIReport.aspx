...
<head>
    ...
    <script type="text/javascript">
        ...
        var allLanguages = 'CHS,CHT,CSY,DAN,DEU,ELL,ENU,ESN,FIN,FRA,HUN,ITA,JPN,KOR,NLD,NOR,PLK,PTB,PTG,RUS,SVE,TRK';
        var clickedObj;
        ...
        var resultsCount = 0;
        var msazureLink = 'https://msazure.visualstudio.com/Configmgr/_workitems/edit/';
        var ceapexLink = 'https://msazure.visualstudio.com/Configmgr/_workitems/edit/';

        var relatedAssemblyAndFullTypeDic;
        //class names
        ...
        function CreateTag3(text, actionClass, tagTitle, value) {
            ...
            return "<span class='Tag " + actionClass + "' title='" + tagTitle + "' value='" + value + "' value1='" + text + "'>" + text + "</span>";
        }
        ...
        function DoQuery(pageIndex, flag) {
            ...
            //selected Result
            //SelectedResult = GetRangeList(SISelectedResultTags1, 4);
            ...
            if (getUrlParam('fulltypename') != null) {
                ...
                $(SISelectedFullTypeTags).each(function () {
                    ...
                    arrayTemp.push($(this).attr('value1'));
                });
            ...
            if (getUrlParam('reviewstatus') != null) {
                SelectedReviewed = getUrlParam('reviewstatus');
            } else {
            ...
            if (resultid == '' && showLatest && (SelectedRules == 'All' || SelectedRules.indexOf(',') != -1)) {
                ...
                if (pageIndex == 0) {
                    ...
                    $.ajax({
                        ...
                        data: "{buildNO:\"" + SelectedBuild + "\",buildLan:\"" + SelectedBuildLanguage + "\",osType:\"" + SelectedOSType + "\",osLanguage:\""
                            ...
                            + "\",ReviewedType:\"" + SelectedReviewed + "\",resultid:\"" + resultid + "\",searchDateTime:\""
                            + searchDateTime + "\",getLatest:\"" + showLatest + "\",latestDays:\"" + latestDays + "\"}",
                        dataType: 'json',
                        error: function (data) {
                        ...
                        success: function (data) {
                            ...
                            if (data.d == 0) {
                                ...
                                ShowResults(SelectedBuild, SelectedBuildLanguage, SelectedOSType, SelectedOSLanguage, SelectedRules, SelectedResult,
                                    SelectedAssembly, SelectedTypes, pageIndex, alias, pagetitle, reviewComments, SelectedReviewed, SelectedSortBy, resultid,
                                    searchDateTime, pageResults, 0, showLatest, latestDays);
                            }
                        }
                    });
                }
            ...
            if (flag != 0) {
                console.log(1 + '----' + pageDiffentReviewStatusResultsCount);
                ...
                console.log(2 + '----' + pageDiffentReviewStatusResultsCount);
                ShowResults(SelectedBuild, SelectedBuildLanguage, SelectedOSType, SelectedOSLanguage, SelectedRules, SelectedResult,
                    SelectedAssembly, SelectedTypes, pageIndex, alias, pagetitle, reviewComments, SelectedReviewed, SelectedSortBy, resultid,
                    searchDateTime, pageResults, diffentReviewStatusResultsCount, showLatest, latestDays);
                console.log(3 + '----' + pageDiffentReviewStatusResultsCount);
            }
        }
        function ShowResults(SelectedBuild, SelectedBuildLanguage, SelectedOSType, SelectedOSLanguage, SelectedRules, SelectedResult,
            SelectedAssembly, SelectedTypes, pageIndex, alias, pagetitle, reviewComments, SelectedReviewed, SelectedSortBy, resultid,
            searchDateTime, pageResults, diffentReviewStatusResultsCount, showLatest, latestDays) {
            $.ajax({
                beforeSend: function () {
                ...
                data: "{buildNO:\"" + SelectedBuild + "\",buildLan:\"" + SelectedBuildLanguage + "\",osType:\"" + SelectedOSType + "\",osLanguage:\""
                    ...
                    + "\",reviewComments:\"" + reviewComments + "\",ReviewedType:\"" + SelectedReviewed
                    ...
                    + "\",getLatest:\"" + showLatest + "\",latestDays:\"" + latestDays + "\"}",
                dataType: 'json',
                error: function (data) {
                    $(SIBtnQuery).removeAttr("disabled");
        ...
        function showResultDetailInfo(resultID) {
            $.ajax({
                ...
                url: 'CaptureUIsService.asmx/GetResultDetailInfo',
                contentType: "application/json;charset=utf-8",
                type: 'POST',
        ...
        function showResultScreenshot(resultID) {
            $.ajax({
                ...
                url: 'CaptureUIsService.asmx/GetScreenshotPath',
                contentType: "application/json;charset=utf-8",
                type: 'POST',
                data: "{resultID:\"" + resultID + "\"}",
                dataType: 'json',
        ...
        function UpdateTypesTag() {
            ...
            $(SIFullTypeListContainer).html('<span class="Tag TagFullTypeAll" value="All" value1="All">All</span>');
            if (arrayTemp.length == 0) {
                return;
            }
            else if (arrayTemp[0] == "All") {
        ...
        function GetSelectedValues(Tags, NullWarningText, AllTagsSelector) {
            var arrayTemp = new Array();
            $(Tags).each(function () {
                arrayTemp.push($(this).text().replace(/(\r\n|\n|\r)/gm, ""));
            });
        ...
        function ShowRelatedBugs(resultID, resultRuleName, resultLanguage) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var bugTypes = ['Same Rule and Same Language',
                        'Same Rule and core code',
                        'Same Rule and different Language',
                        'Different Rule and same Language',
                        'Different Rule and different Language'];
                    var colorSetting = ['Red', 'GoldenRod', 'Green', 'Black'];
                    var bugLinkSetting = [ceapexLink, msazureLink];
                    var bugSummary = new Array();
                    bugSummary[0] = new Array();
                    bugSummary[1] = new Array();
                    bugSummary[2] = new Array();
                    bugSummary[3] = new Array();
                    bugSummary[4] = new Array();

                    for (var item of data.d) {
                        if (item.title.search(resultRuleName) == -1) {
                            if (item.title.search(resultLanguage) == -1) {
                                bugSummary[4].push(item);
                            } else {
                                bugSummary[3].push(item);
                            }
                        } else {
                            if (item.title.search(resultLanguage) != -1) {
                                bugSummary[0].push(item);
                            } else if (item.title.search('ENU') != -1) {
                                bugSummary[1].push(item);
                            } else {
                                bugSummary[2].push(item);
                            }
                        }
                    }

                    var seperatorLine = '================';
                    //{0} - bug link site; {1} - bug id; {2} - color
                    var bugLinkFormatString = '<a href="{0}{1}/" target="_blank" style="color:{2};">{1}</a>';
                    //{0} - color; {1} - display value1; {2} - display value2
                    var tdFormatString = '<tr><td style="padding: 0px 5px;color: {0};">{1}</td><td style="padding: 0px 5px;color: {0};">{2}</td></tr>';
                    var newRow, bugLink, bugLinkSettingIndex;
                    var $RelatedBugsTable = $('#RelatedBugsTableTemplate').clone();
                    for (var bugSummaryIndex = 0; bugSummaryIndex < bugSummary.length; bugSummaryIndex++) {
                        if (bugSummary[bugSummaryIndex].length > 0) {
                            newRow = tdFormatString.format('black', bugTypes[bugSummaryIndex], seperatorLine);
                            $RelatedBugsTable.append(newRow);
                            for (var item of bugSummary[bugSummaryIndex]) {
                                bugLinkSettingIndex = item.bug_id.toString().length - 6;
                                bugLink = bugLinkFormatString.format(bugLinkSetting[bugLinkSettingIndex], item.bug_id, colorSetting[item.status]);
                                newRow = tdFormatString.format(colorSetting[item.status], bugLink, item.title);
                                $RelatedBugsTable.append(newRow);
                            }
                        }
                    }
        ...
        function attachBugID(bugIDs, resultID) {
            $.ajax({
                url: 'CaptureUIsService.asmx/LinkBug',
                contentType: "application/json;charset=utf-8",
                type: 'POST',
        ...
        function ShowRelatedAssemblyInfo(resultID) {
            $.ajax({
                ...
                success: function (data) {
                    if (data.d == null || data.d.length == 0) {
                        $('#AssemblyInfoDialog').dialog('close');
                        alert("No related assemblies found!");
                        return;
                    }
                    ...
                    for (var item of data.d) {
                        var $row = $('<tr></tr>');
                        $('<td></td>').css({ padding: '0 5px 0 5px' }).text(item.AssemblyName).appendTo($row);
                        $('<td></td>').css({ padding: '0 5px 0 5px' }).text(item.FullTypeName).appendTo($row);
        ...
        function showTestCoverageResult(abovebuildNo, buildLanguages, ruleIDs, selectedAssemblyName, selectFullTypeName) {
            $.ajax({
                beforeSend: function () {
                ...
                url: 'CaptureUIsService.asmx/GetLatestResultSummary',
                contentType: "application/json;charset=utf-8",
                type: 'POST',
                data: "{aboveBuildNo:\"" + abovebuildNo + "\",buildLanguages:\"" + buildLanguages + "\",ruleIDs:\"" + ruleIDs
                    + "\",assemblyName:\"" + selectedAssemblyName + "\",fullTypeName:\"" + selectFullTypeName + "\"}",
                dataType: 'json',
                error: function (data) {
                    console.log(data);
                    $('#TestCoverageDialog').dialog('close');
                    alert("Fail to call GetLatestResultSummary!");
                },
                success: function (data) {
                    if (data.d == null) {
                        $('#TestCoverageDialog').dialog('close');
                        alert("Error occurs when call GetLatestResultSummary!");
                        return;
                    }
                    var dic = data.d;
                    if (buildLanguages == 'All')
                        buildLanguages = allLanguages;
                    if (ruleIDs == 'All')
                        ruleIDs = availableRuleIDs;
                    console.log('ruleIDs-' + ruleIDs);

                    var currentInfo = "Build since: " + abovebuildNo + " | Assembly: " + selectedAssemblyName + " | FullTypeName: " + selectFullTypeName;
                    ...
                    var languageArr = buildLanguages.split(',');
                    ...
                    var $row, $cell;
                    var colorGreen = 'Green', colorGoldenRod = 'GoldenRod', colorRed = 'Red', colorGrey = 'Grey', resultNotRun = 'Not Run';
                    var href = 'http://{0}/SulpHurReports/CapturedUIReport.aspx?buildlanguage={1}&rule={2}&assembly={3}&fulltypename={4}&showlatest=buildno';
                    var hrefFail = 'http://{0}/SulpHurReports/CapturedUIReport.aspx?buildlanguage={1}&rule={2}&assembly={3}&fulltypename={4}&resulttype=Fail';
                    var updatedHref;
                    var ruleIDArray = ruleIDs.split(',');
                    for (var key of ruleIDArray) {
                        $row = $('<tr></tr>');
                        $('<td style="border: 1px solid">' + availableRules[key].RuleName + '</td>').css({ padding: '0 5px 0 5px' }).appendTo($row);
                        languageArr.forEach((item, index, arr) => {
                            $cell = $('<td style="border: 1px solid"></td>').css({ padding: '0 5px 0 5px' });
                            if (!dic.hasOwnProperty(availableRules[key].RuleID) || dic[availableRules[key].RuleID][item] == null || dic[availableRules[key].RuleID][item] == '') {
                                $('<td style="border: 1px solid"></td>').css({ padding: '0 5px 0 5px', color: colorGrey }).text(resultNotRun).appendTo($row);
                            } else {
                                if (dic[availableRules[key].RuleID][item].ResultType == 'Fail') {
                                    updatedHref = hrefFail.format(serverName, item, availableRules[key].RuleName, selectedAssemblyName, selectFullTypeName);
                                    $('<a target="_blank"></a>').attr("href", updatedHref).css({ color: colorRed }).text('[F]:' + dic[availableRules[key].RuleID][item].MainBuildVersion).appendTo($cell);
                                } else if (dic[availableRules[key].RuleID][item].ResultType == 'Warning') {
                                    updatedHref = href.format(serverName, item, availableRules[key].RuleName, selectedAssemblyName, selectFullTypeName);
                                    $('<a target="_blank"></a>').attr("href", updatedHref).css({ color: colorGoldenRod }).text('[W]:' + dic[availableRules[key].RuleID][item].MainBuildVersion).appendTo($cell);
                                } else {
                                    updatedHref = href.format(serverName, item, availableRules[key].RuleName, selectedAssemblyName, selectFullTypeName);
                                    $('<a target="_blank"></a>').attr("href", updatedHref).css({ color: colorGreen }).text('[P]:' + dic[availableRules[key].RuleID][item].MainBuildVersion).appendTo($cell);
                                }
                                $cell.appendTo($row);
                            }
                        })
        ...
        function ChangeAssemblySelect() {
            var selectedAssemblyName = $('#RelatedAssemblySelect').val();
            $('#RelatedFullTypeSelect option').remove();
            var addedFirstFullType = false;
            var relatedFullTypeList;
            for (var i = 0; i < relatedAssemblyAndFullTypeDic[selectedAssemblyName].length; i++) {
                if (addedFirstFullType) {
                    relatedFullTypeList = "<option value='" + data.d[i].AssemblyName + "'>" + data.d[i].AssemblyName + "</option>";
                } else {
                    relatedFullTypeList = "<option value='" + value.AssemblyName + "' selected='selected'>" + value.AssemblyName + "</option>";
                    addedFirstFullType = true;
                }
                $('#RelatedFullTypeSelect').append(relatedFullTypeList);
            }
   ...
   <div id="FileMultiBugWithAttachForm" class="PopupWindow" oncontextmenu="return false;">
    ...
    <div id="ExportTestCoverageForm" class="PopupWindow" oncontextmenu="return false;" style="width:950px;">
        ...
        <div>
            ...
            <select id="AboveBuildNoSelect"></select>
        </div>
        <div>
            <span style="float: left; margin: 0 10px 0 0px;">Related assembly:</span>
            <select id="RelatedAssemblySelect" onchange="ChangeAssemblySelect();"></select>
        </div>
        <div>
            <span style="float: left; margin: 0 10px 0 0px;">Related full type:</span>
            <select id="RelatedFullTypeSelect"></select>
        </div>
        <div class="ButtonPanel">
            <input hotkey="ENTER" class="Modern-Button" id="btnExportTestCoverageOK" type="button" value="OK" style="margin: 0px 0 0 0px;" />
            <input hotkey="ESC" class="Modern-Button" id="btnExportTestCoverageCancel" type="button" value="Cancel" style="margin: 0px 0 0 20px;" />
        </div>
    </div>
...
<%--                <tr>
                        ...
                        <td id="SC_BuildType">
                            <span class="Tag TagAll">All</span>
                            <span class="Tag TagCommon">Main</span>
                            <span class="Tag TagCommon">Private</span>
                        </td>
                        <td>
                        </td>
                    </tr>--%>
                    ...
                    <tr>
                        ...
                        <td id="SC_FullTypeName">
                            ...
                            <div class="LargeListTd">
                                ...
                                <span class="Tag TagFullTypeAll" value="All" value1="All">All</span>
                            </div>
                        </td>
                        <td class="td-topAlign">
                            <!--
...
<%--            <input type="checkbox" id="EnableDelete" value="Enable Delete" />Enable Delete
            ...
            <input type="checkbox" id="showLatest"  value="Show Latest Build" name="showLatest" checked="checked" />Show Latest Result
            <%--<input type="checkbox" id="showLatest"  value="Show Latest Build" name="showLatest" onclick="EnableSortBy(this.checked)"/>Latest Result --%>
