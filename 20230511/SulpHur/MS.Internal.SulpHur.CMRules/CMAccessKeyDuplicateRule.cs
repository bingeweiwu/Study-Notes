...
{
    public class CMAccessKeyDuplicateRule : UIComplianceRuleBase
    {
        public override string Name
        {
            get { return @"Access Key Duplicate Rule"; }
        }

        public override string Description
        {
            get { return @"This rule looks for duplicated access keys."; }
        }

        public CMAccessKeyDuplicateRule()
        {
            ruleProperties = new AccessKeyDuplicateRuleProperties();
        ...
        public override List<UIComplianceResultBase> UIVerify(ICollection<ElementInformation> controls)
        {
            ...
            if (lan != "DEU" && lan != "FRA" && lan != "RUS" && lan != "ENU")
            {
                results.Add(new UIComplianceResultBase(ResultType.Warning, $"{Name} not works on {lan}. It will set to warning.", Name));
                return results;
            }
            ...
            else
            {
                ...
                List<ElementInformation> TextControls = new List<ElementInformation>();
                ...
                //v-yiwzha: Filter same Name with previous control which have same access key
                foreach (ElementInformation ei in filteredControls)
                {
                    ElementInformation previous = CheckPreviousIndexControl(ei);
                    if (previous != null)
                    {
                        if (previous.ControlType == ControlType.Text)
                        {
                            TextControls.Add(ei);
                        }
                    }
                ...
                foreach (ElementInformation ei in TextControls)
                {
                    ElementInformation previous = CheckPreviousIndexControl(ei);
                    if (previous != null)
                    {
                        if (ei.Name == previous.Name)
                        {
                            filteredControls.Remove(ei);
                        }
                    }
                }
		...
		foreach (ElementInformation ei in filteredControls)
                {
                    if (ei.PreviousSibling != null) 
                    {
                        if (ei.PreviousSibling.ControlType == ControlType.Text && ei.PreviousSibling.Name == ei.Name)
                            TextControls.Add(ei.PreviousSibling);
                    }
                    if (ei.NextSibling != null)
                    {
                        if (ei.NextSibling.ControlType == ControlType.Text && ei.NextSibling.Name == ei.Name)
                            TextControls.Add(ei.NextSibling);
                    }
                }
                ...
                //                        //    continue;
                //                        if (IsAccessKeyControl(control) && string.IsNullOrEmpty(control.AccessKey))
                //                        {
                //#if DEBUG
                        ...
                        else
                        {
                            ...
                            if (control.Name != null)
                            {
                                ...
                                for (int i = 0; i < control.Name.Length; i++)
                                {
                                    ...
                                    if (lan == "CHS" || lan == "CHT" || lan == "JPN" || lan == "KOR")
                                    {
                                        suggestkey = '+';
                                        logMessage = string.Empty;
                                        break;
        ...
        private bool IsAccessKeyControl(ElementInformation control)
        {
            ...
            //v-edy: bug490518:-result ID 6442205, these automationIDs dont need check accesskey
            if ("_headlineLabel" == control.AutomationId || "TitleBar" == control.AutomationId || "_pageLogo" == control.AutomationId)
                return false;
        ...
        private AccessKeyDuplicateRuleProperties ruleProperties;
    ...
    public class AccessKeyDuplicateRuleProperties
    {
