...
namespace MS.Internal.SulpHur.SulpHurService
{
    public class UIContentVerification : VerifyOperations
    {
        ...
        private Thread cacheResultInfoThread;
        ...
        public UIContentVerification()
        {
            ...
            cacheResultInfoThread = new Thread(TimingCacheResultInfo);
...
FROM (
 ...
 AND [TOTAL].[UIName]=[GroupTable].[UIName]) AS newT  ORDER BY rowNum";
            ...
            {
                Trace.WriteLine("time after 19:00 on Thursday, isEmpty = "+isEmpty.ToString());
                isWithinSchedule = true;
            }
            else if (beijingTime.DayOfWeek == DayOfWeek.Friday && beijingTime.Hour < 5)
            {
                Trace.WriteLine("time before 5:00 on Friday, isEmpty = " + isEmpty.ToString());
                isWithinSchedule = true;
            }

            if (beijingTime.DayOfWeek == DayOfWeek.Thursday && beijingTime.Hour == 18  && !isEmpty)
            {
                Trace.WriteLine("start clear ResultIDbyAlias and ResultsInfoByAlias ");                
                TruncateCacheTable(connStr, "TRUNCATE TABLE ResultIDbyAlias");
                TruncateCacheTable(connStr, "TRUNCATE TABLE ResultsInfoByAlias");
                Trace.WriteLine("clear ResultIDbyAlias and ResultsInfoByAlias complete");
                isEmpty = true;
            }
            ...
            if (isWithinSchedule && isEmpty)
            {
                ...
                {
                    Trace.WriteLine("start cache " + aliases[i] + "data");
                    ExecuteScalar(connStr, insertSqls[i]);
                    Trace.WriteLine("cache " + aliases[i] + " data complete");
                }
            ...
            {
                Trace.WriteLine("The current time is not within the specified time range or data table has been filled in ");
            }
        }
        //
